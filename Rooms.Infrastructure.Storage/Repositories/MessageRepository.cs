...
/// </summary>
public class MessageRepository(MongoDbContext context) : RepositoryBase<MessageModel, Message>(c => c.Id, context.Messages), IMessageRepository
{
    /// <inheritdoc/>
    /// <summary> 
    ...
    public Task AddAsync(Message aggregate, CancellationToken cancellationToken = default)
    {
        ...
        var model = new MessageModel { Id = aggregate.Id };
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return models
            ...
            .Select(m => Message.FromSnapshot(m.GetSnapshot())) // Преобразуем каждую модель в агрегат с помощью маппера ReportMapper.Map().
