...
public class OwnerTimeLineChangedEventHandler(IPublishEndpoint publish, IScopedContext context)
    : AfterSaveNotificationHandler<ViewerTimeLineChangedEvent>
{
    ...
    protected override async Task Execute(ViewerTimeLineChangedEvent @event, CancellationToken cancellationToken)
    {
        ...
        await publish.Publish<RoomBaseEvent>(new TimeLineEvent { TimeLine = @event.Viewer.TimeLine.Ticks }, cancellationToken);
    }
}