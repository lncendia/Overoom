...
/// </summary>
public class CommentRepository(MongoDbContext context) : RepositoryBase<CommentModel, Comment>(c => c.Id, context.Comments), ICommentRepository
{
    /// <inheritdoc/>
    /// <summary> 
    /// Асинхронно добавляет новый агрегат. 
    /// </summary> 
    public Task AddAsync(Comment aggregate, CancellationToken cancellationToken = default)
    {
        // Преобразуем агрегат в модель данных, которая будет использоваться для хранения данных в базе данных.
        var model = new CommentModel { Id = aggregate.Id };
    ...
    /// </summary>
    public Task AddRangeAsync(IReadOnlyList<Comment> aggregates, CancellationToken cancellationToken = default)
    {
    ...
    /// </summary> 
    public Task UpdateAsync(Comment aggregate, CancellationToken cancellationToken = default)
    {
    ...
    /// </summary>
    public Task UpdateRangeAsync(IReadOnlyList<Comment> aggregates, CancellationToken cancellationToken = default)
    {
        // Выполняем код для каждого агрегата
        foreach (var aggregate in aggregates)
        {
    ...
    /// </summary>
    public async Task<int> DeleteRangeAsync(ISpecification<Comment, ICommentSpecificationVisitor> specification,
        CancellationToken cancellationToken = default)
    {
        // Получаем базовый запрос к коллекции Comment из контекста.
    ...
    public async Task<Comment?> GetAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        return Comment.FromSnapshot(model.GetSnapshot());
    }
    ...
    /// Асинхронно выполняет поиск агрегатов, удовлетворяющих указанной спецификации, с возможностью сортировки, пропуска и взятия определенного количества. 
    /// </summary> 
    public async Task<IReadOnlyList<Comment>> FindAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return models
            ...
            .Select(m => Comment.FromSnapshot(m.GetSnapshot())) // Преобразуем каждую модель в агрегат с помощью маппера ReportMapper.Map().
    ...
    /// </summary> 
    public async Task<int> CountAsync(ISpecification<Comment, ICommentSpecificationVisitor>? specification,
        CancellationToken cancellationToken = default)
    {
        // Получение запроса на выборку из базы данных
        var query = Collection.AsQueryable();
