...
/// </summary>
public class UserRepository(MongoDbContext context) : RepositoryBase<UserModel, User>(c => c.Id, context.Users), IUserRepository
{
    /// <inheritdoc/>
    /// <summary> 
    ...
    public Task AddAsync(User aggregate, CancellationToken cancellationToken = default)
    {
        ...
        var model = new UserModel { Id = aggregate.Id };
    ...
    /// </summary>
    public Task AddRangeAsync(IReadOnlyList<User> aggregates, CancellationToken cancellationToken = default)
    {
    ...
    /// </summary>
    public Task UpdateRangeAsync(IReadOnlyList<User> aggregates, CancellationToken cancellationToken = default)
    {
        // Выполняем код для каждого агрегата
        foreach (var aggregate in aggregates)
        {
    ...
    /// </summary>
    public async Task<int> DeleteRangeAsync(ISpecification<User, IUserSpecificationVisitor> specification,
        CancellationToken cancellationToken = default)
    {
        // Получаем базовый запрос к коллекции User из контекста.
    ...
    public async Task<User?> GetAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        return User.FromSnapshot(model.GetSnapshot());
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return models
            ...
            .Select(m => User.FromSnapshot(m.GetSnapshot())) // Преобразуем каждую модель в агрегат с помощью маппера ReportMapper.Map().
    ...
    /// </summary> 
    public async Task<int> CountAsync(ISpecification<User, IUserSpecificationVisitor>? specification,
        CancellationToken cancellationToken = default)
    {
        // Получение запроса на выборку из базы данных
        var query = Collection.AsQueryable();
