...
/// </summary>
public class RatingRepository(MongoDbContext context) : RepositoryBase<RatingModel, Rating>(c => c.Id, context.Ratings), IRatingRepository
{
    /// <inheritdoc/>
    /// <summary> 
    /// Асинхронно добавляет новый агрегат. 
    /// </summary> 
    public Task AddAsync(Rating aggregate, CancellationToken cancellationToken = default)
    {
        // Преобразуем агрегат в модель данных, которая будет использоваться для хранения данных в базе данных.
        var model = new RatingModel { Id = aggregate.Id };
    ...
    /// </summary>
    public Task AddRangeAsync(IReadOnlyList<Rating> aggregates, CancellationToken cancellationToken = default)
    {
    ...
    /// </summary> 
    public Task UpdateAsync(Rating aggregate, CancellationToken cancellationToken = default)
    {
    ...
    /// </summary>
    public Task UpdateRangeAsync(IReadOnlyList<Rating> aggregates, CancellationToken cancellationToken = default)
    {
        // Выполняем код для каждого агрегата
        foreach (var aggregate in aggregates)
        {
    ...
    /// </summary>
    public async Task<int> DeleteRangeAsync(ISpecification<Rating, IRatingSpecificationVisitor> specification,
        CancellationToken cancellationToken = default)
    {
        // Получаем базовый запрос к коллекции Rating из контекста.
    ...
    /// </summary> 
    public async Task<Rating?> GetAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        return Rating.FromSnapshot(model.GetSnapshot());
    }
    ...
    /// Асинхронно выполняет поиск агрегатов, удовлетворяющих указанной спецификации, с возможностью сортировки, пропуска и взятия определенного количества. 
    /// </summary> 
    public async Task<IReadOnlyList<Rating>> FindAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return models
            ...
            .Select(m => Rating.FromSnapshot(m.GetSnapshot())) // Преобразуем каждую модель в агрегат с помощью маппера ReportMapper.Map().
    ...
    /// </summary>
    public async Task<Rating?> FirstOrDefaultAsync(
        ISpecification<Rating, IRatingSpecificationVisitor>? specification,
        CancellationToken cancellationToken = default
    )
    {
    ...
    /// </summary> 
    public async Task<int> CountAsync(ISpecification<Rating, IRatingSpecificationVisitor>? specification,
        CancellationToken cancellationToken = default)
    {
        // Получение запроса на выборку из базы данных
        var query = Collection.AsQueryable();
